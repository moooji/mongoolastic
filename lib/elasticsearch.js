...
ElasticsearchProvider.prototype.indexDoc = function(id, doc, type, index, callback) {
  ...
  return Bluebird.resolve([id, doc, type, index])
    .spread((id, doc, type, index) => {
      ...
      return this.client.index({index, type, id, body: doc, refresh: true});
    })
    .nodeify(callback);
};
...
ElasticsearchProvider.prototype.bulkIndexDoc = function(id, doc, type, index, callback) {
  ...
  return Bluebird.resolve([id, doc, type, index])
    .spread((id, doc, type, index) => {
      ...
      // Add action and document to buffer
