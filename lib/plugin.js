...
function Mongoolastic() {
  ...
  this.populationTrees = new Map();
  this.indexPaths = new Map();
  ...
  return Bluebird.resolve()
    .then(() => {
      ...
      self.registeredModels.forEach((model) => {
        ...
        self.mappings.set(model.modelName, mapping);

        // Population trees
        const populationTree = helpers.renderPopulationTree(model.schema, self.populationModels);
        self.populationTrees.set(model.modelName, populationTree);

        // Index paths
        const indexPaths = helpers.renderIndexPaths(model.schema, self.populationModels);
        self.indexPaths.set(model.modelName, indexPaths);
      });
    })
    .nodeify(callback);
};
...
Mongoolastic.prototype.indexDoc = function(doc, callback) {
  ...
  return Bluebird.resolve(doc)
    .then((doc) => {
      ...
      const isRegistered = self.registeredModels.has(modelName);
      ...
      const id = doc.id;
      const esDoc = helpers.renderDoc(doc, self.indexPaths.get(modelName), []);
      return self.es.indexDoc(id, esDoc, modelName, self.index);
    })
    .nodeify(callback);
};
...
Mongoolastic.prototype.getMappings = function() {
  ...
  this.mappings.forEach((mapping, modelName) => {
    mergedMappings[modelName] = mapping;
  });
