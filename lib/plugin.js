...
function Mongoolastic() {
  ...
  this.mappings = {};
  this.schemas = [];
  this.registeredPopulations = new Map();
 ...
 * @returns {Promise}
 */
Mongoolastic.prototype.registerPopulation = function(model, fields, callback) {

  const self = this;

  // Check if fields are provided
  // or if the fields argument is actually the callback
  if (callback === undefined && _.isFunction(fields)) {
    callback = fields;
    fields = undefined;
  }
  ...
  return Bluebird.resolve(model)
    .then((model) => {
      ...
      // Register population
      self.registeredPopulations.set(model.modelName, {model, fields});
...
Mongoolastic.prototype.updateMappings = function(callback) {
  ...
  return Bluebird.resolve(model)
    .then((model) => {
      ...
      // TODO iterate through all models and populations
      // and render mappings
