  ...
  return Bluebird.resolve()
    .then(() => {
      ...
      self.registeredModels.forEach((model) => {
        ...
        const parsedSchema = helpers.parseSchema(model.schema, self.populationModels);

        if(parsedSchema.mapping) {
          mappings = _.merge(mappings, {[model.modelName]: parsedSchema.mapping});
        }
      });
